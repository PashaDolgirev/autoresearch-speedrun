...
# -----------------------------------------------------------------------------
# Framework contract (autoresearch-speedrun harness) — DO NOT MODIFY
# Each run is a fixed-wall-clock experiment: we train for TIME_BUDGET_S seconds
# of measured time (excluding compile + the 10-step warmup) and then run a
# final val_loss eval. SEED comes from env so the harness can do paired-seed
# comparisons; same SEED on the same machine reproduces the same final val_loss.
TIME_BUDGET_S = float(os.environ.get('TIME_BUDGET_S', 90.0))
...
for step in range(train_steps + 1):
    ...
    if step == 10:
        training_time_ms = 0
        t0 = time.perf_counter()
    # Framework contract (DO NOT MODIFY): stop after TIME_BUDGET_S seconds of measured
    # training and force the final val_loss eval (which is gated on `last_step`).
    elapsed_s = (training_time_ms + 1000.0 * (time.perf_counter() - t0)) / 1000.0
    if step > 10 and elapsed_s >= TIME_BUDGET_S:
        last_step = True
